...
Reads all per-round client energy logs and produces:
...
def _load_energy_logs():
    ...
    Returns:
        list[dict]: one entry per client per round.
    """
    energy_entries = []
...
def aggregate_energy_logs():
    ...
    mixing results from different experiments (when logs are not cleared).
    """
    ...
    if not filtered:
        ...
        print(
            f"[SERVER] No energy logs for dataset={dataset}, "
            f"mode={mode}, variant={variant}"
        )
        return summary

    # Aggregate per-client totals across all rounds
    client_totals = {}
    for e in filtered:
    ...
    print(
        f"[SERVER] Energy totals | dataset={dataset}, mode={mode}, "
        f"variant={variant}, total={total_energy:.4f} J, "
        f"mean_per_client={mean_energy:.4f} J"
    )
